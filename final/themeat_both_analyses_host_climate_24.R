...
resid_soil_hc <- subset(merge_soil, select = c(5,7,10))
...
resid_foliar_hc <- subset(merge_foliar, select = c(3,6,9))
...
p_full <- lm(path.diversity ~ pH + soil, data = merge_soil)
summary(p_full)
p_resid_d <- resid(p_full)
p_climate <- lm(p_resid_d ~ climate + species, data = merge_soil)
summary(p_climate)
p_resid_d <- as.data.frame(p_resid_d)
resid_soil_hc <- cbind(resid_soil_hc, p_resid_d)

pd <- ggplot(resid_soil_hc, aes(x = climate, y = p_resid_d)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
pd
...
e_full <- lm(ecm.diversity ~ pH + soil, data = merge_soil)
summary(e_full)
e_resid_d <- resid(e_full)
e_climate <- lm(e_resid_d ~ climate + species + climate:species, data = merge_soil)
summary(e_climate)
e_resid_d <- as.data.frame(e_resid_d)
resid_soil_hc <- cbind(resid_soil_hc, e_resid_d)

ed <- ggplot(resid_soil_hc, aes(x = climate, y = e_resid_d)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
ed
...
a_full <- lm(am.diversity ~ pH + soil, data = merge_soil)
summary(a_full)
a_resid_d <- resid(a_full)
a_climate <- lm(a_resid_d ~ climate + species, data = merge_soil)
summary(a_climate)
a_resid_d <- as.data.frame(a_resid_d)
resid_soil_hc <- cbind(resid_soil_hc, a_resid_d)

ad <- ggplot(resid_soil_hc, aes(x = climate, y = a_resid_d)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
ad
...
s_full <- lm(sap.diversity ~ pH + soil, data = merge_soil)
summary(s_full)
s_resid_d <- resid(s_full)
s_climate <- lm(s_resid_d ~ climate + species, data = merge_soil)
summary(s_climate)
s_resid_d <- as.data.frame(s_resid_d)
resid_soil_hc <- cbind(resid_soil_hc, s_resid_d)

sd <- ggplot(resid_soil_hc, aes(x = climate, y = s_resid_d)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
sd
...
p_full <- lm(pathogen ~ pH + soil, data = merge_soil)
summary(p_full)
p_resid_cwm <- resid(p_full)
p_climate <- lm(p_resid_cwm ~ climate + species, data = merge_soil)
summary(p_climate)
p_resid_cwm <- as.data.frame(p_resid_cwm)
resid_soil_hc <- cbind(resid_soil_hc, p_resid_cwm)

p_cwm <- ggplot(resid_soil_hc, aes(x = climate, y = p_resid_cwm)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
p_cwm
...
e_full <- lm(ecm ~ pH + soil, data = merge_soil)
summary(e_full)
e_resid_cwm <- resid(e_full)
e_climate <- lm(e_resid_cwm ~ climate + species, data = merge_soil)
summary(e_climate)
e_resid_cwm <- as.data.frame(e_resid_cwm)
resid_soil_hc <- cbind(resid_soil_hc, e_resid_cwm)

e_cwm <- ggplot(resid_soil_hc, aes(x = climate, y = e_resid_cwm)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
e_cwm
...
a_full <- lm(am ~ pH + soil, data = merge_soil)
summary(a_full)
a_resid_cwm <- resid(a_full)
a_climate <- lm(a_resid_cwm ~ climate + species, data = merge_soil)
summary(a_climate)
a_resid <- as.data.frame(a_resid_cwm)
resid_soil_hc <- cbind(resid_soil_hc, a_resid_cwm)

a_cwm <- ggplot(resid_soil_hc, aes(x = climate, y = a_resid_cwm)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
a_cwm
...
s_full <- lm(saprobe ~ pH + soil, data = merge_soil)
summary(s_full)
s_resid_cwm <- resid(s_full)
s_climate <- lm(s_resid_cwm ~ climate + species, data = merge_soil)
summary(s_climate)
s_resid_cwm <- as.data.frame(s_resid_cwm)
resid_soil_hc <- cbind(resid_soil_hc, s_resid_cwm)

s_cwm <- ggplot(resid_soil_hc, aes(x = climate, y = s_resid_cwm)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
s_cwm
...
p_full <- lm(path.host.spe ~ pH + soil, data = merge_soil)
summary(p_full)
p_resid_spe <- resid(p_full)
p_climate <- lm(p_resid_spe ~ climate + species, data = merge_soil)
summary(p_climate)
p_resid_spe <- as.data.frame(p_resid_spe)
resid_soil_hc <- cbind(resid_soil_hc, p_resid_spe)

p_spe <- ggplot(resid_soil_hc, aes(x = climate, y = p_resid_spe)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
p_spe
...
e_full <- lm(ecm.host.spe ~ pH + soil, data = merge_soil)
summary(e_full)
e_resid_spe <- resid(e_full)
e_climate <- lm(e_resid_spe ~ climate + species, data = merge_soil)
summary(e_climate)
e_resid_spe <- as.data.frame(e_resid_spe)
resid_soil_hc <- cbind(resid_soil_hc, e_resid_spe)

e_spe <- ggplot(resid_soil_hc, aes(x = climate, y = e_resid_spe)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
e_spe
...
a_full <- lm(am.host.spe ~ pH + soil, data = merge_soil)
summary(a_full)
a_resid_spe <- resid(a_full)
a_resid_spe <- as.data.frame(a_resid_spe)
#a_climate <- lm(a_resid_spe ~ climate + species, data = merge_soil)
resid_soil_hc <- merge(resid_soil_hc, a_resid_spe, by = 0, all = TRUE)

#summary(a_climate)

a_spe <- ggplot(resid_soil_hc, aes(x = climate, y = a_resid_spe)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
a_spe
...
s_full <- lm(sap.host.spe ~ pH + soil, data = merge_soil)
summary(s_full)
s_resid_spe <- resid(s_full)
s_climate <- lm(s_resid_spe ~ climate + species, data = merge_soil)
summary(s_climate)
s_resid_spe <- as.data.frame(s_resid_spe)
resid_soil_hc <- cbind(resid_soil_hc, s_resid_spe)

s_spe <- ggplot(resid_soil_hc, aes(x = climate, y = s_resid_spe)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
s_spe
...
p_full <- lm(path.diversity ~ pH + foliar, data = merge_foliar)
summary(p_full)
p_resid_d <- resid(p_full)
p_climate <- lm(p_resid_d ~ climate + species, data = merge_foliar)
summary(p_climate)
p_resid_d <- as.data.frame(p_resid_d)
resid_foliar_hc <- cbind(resid_foliar_hc, p_resid_d)

pd <- ggplot(resid_foliar_hc, aes(x = climate, y = p_resid_d)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
pd
...
s_full <- lm(sap.diversity ~ pH + foliar, data = merge_foliar)
summary(s_full)
s_resid_d <- resid(s_full)
s_climate <- lm(s_resid_d ~ climate + species, data = merge_foliar)
summary(s_climate)
s_resid_d <- as.data.frame(s_resid_d)
resid_foliar_hc <- cbind(resid_foliar_hc, s_resid_d)

sd <- ggplot(resid_foliar_hc, aes(x = climate, y = s_resid_d)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
sd
##THE END OF PT1: DIVERSITY##
...
mod_full <- model_avg$coefmat.full
mod_full
write.csv(mod_full, "\\Users\\abbey\\Desktop\\2023-2024\\sgfh manuscript\\data\\diversity models\\foliar.path.cwm.csv")
write.csv(aic_p, "\\Users\\abbey\\Desktop\\2023-2024\\sgfh manuscript\\data\\diversity models\\foliar.path.cwm.aic.csv")

p_full <- lm(pathogen ~ pH + foliar, data = merge_foliar)
summary(p_full)
p_resid_cwm <- resid(p_full)
p_climate <- lm(p_resid_cwm ~ climate + species, data = merge_foliar)
summary(p_climate)
p_resid_cwm <- as.data.frame(p_resid_cwm)
resid_foliar_hc <- cbind(resid_foliar_hc, p_resid_cwm)

p_cwm <- ggplot(resid_foliar_hc, aes(x = climate, y = p_resid_cwm)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
p_cwm
...
s_full <- lm(saprobe ~ pH + foliar, data = merge_foliar)
summary(s_full)
s_resid_cwm <- resid(s_full)
s_climate <- lm(s_resid_cwm ~ climate + species, data = merge_foliar)
summary(s_climate)
s_resid_cwm <- as.data.frame(s_resid_cwm)
resid_foliar_hc <- cbind(resid_foliar_hc, s_resid_cwm)

s_cwm <- ggplot(resid_foliar_hc, aes(x = climate, y = s_resid_cwm)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
s_cwm
...
p_full <- lm(path.host.spe ~ pH + foliar, data = merge_foliar)
summary(p_full)
p_resid_spe <- resid(p_full)
p_climate <- lm(p_resid_spe ~ climate + species, data = merge_foliar)
summary(p_climate)
p_resid_spe <- as.data.frame(p_resid_spe)
resid_foliar_hc <- cbind(resid_foliar_hc, p_resid_spe)

p_spe <- ggplot(resid_foliar_hc, aes(x = climate, y = p_resid_spe)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
p_spe
...
s_full <- lm(sap.host.spe ~ pH + foliar, data = merge_foliar)
summary(s_full)
s_resid_spe <- resid(s_full)
s_climate <- lm(s_resid_spe ~ climate + species, data = merge_foliar)
summary(s_climate)
s_resid_spe <- as.data.frame(s_resid_spe)
resid_foliar_hc <- cbind(resid_foliar_hc, s_resid_spe)

s_spe <- ggplot(resid_foliar_hc, aes(x = climate, y = s_resid_spe)) + geom_point() + geom_smooth(method = "lm", size = 3) + ylab("") + theme_classic(base_size=15) + scale_color_manual(values=c("#993404")) + guides(color = "none") + xlab(" ")
s_spe

##THE END OF PT 3: SPECIFICITY##

##SAVE FILES FOR FIGURES##
saveRDS(resid_foliar_hc, "\\Users\\abbey\\Desktop\\2023-2024\\sgfh manuscript\\rds.files\\resid_foliar_hc_24.rds")
saveRDS(resid_soil_hc, "\\Users\\abbey\\Desktop\\2023-2024\\sgfh manuscript\\rds.files\\resid_soil_hc_24.rds")
